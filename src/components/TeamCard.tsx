    ...
    <div className="border rounded-lg shadow-lg p-4 bg-white flex flex-col gap-4">
      ...
      >
        <RadarChart data={teamRadarData}>
          ...
          <PolarAngleAxis dataKey="attribute" tick={{ fontSize: 10, dy: 6 }} />
          <PolarGrid />
          <PolarRadiusAxis
            domain={[0, 5]}
            tickCount={6}
      ...
      <div className="grid grid-cols-2 gap-x-6 gap-y-8 text-sm font-medium">
        <div>
          <p className="text-gray-700">Ataque: {teamStats.attack.toFixed(2)}</p>
...
export const PlayerListInTeam = ({
  ...
  return (
    <div>
      <h4 className="text-lg font-semibold text-gray-800 mb-2">Jogadores:</h4>
        ...
        {players.map((player) => (
          ...
          >
            ...
            <div className="ml-auto relative">
              <button
                onClick={() =>
                  setOpenDropdown(openDropdown === player.id ? null : player.id)
                }
              ...
              {openDropdown === player.id && (
                <div className="absolute right-0 mt-2 w-48 bg-white border shadow-lg rounded-md z-10">
                  <ul className="max-h-48 overflow-y-auto">
                    {teams
                      ?.flatMap(
                        (team, idx) => (idx !== teamIndex ? team.players : []), // Só inclui jogadores de outros times
                      )
                      .map((swapCandidate) => (
                        <li key={swapCandidate.id}>
                          <button
                            onClick={() => handleSwap(player, swapCandidate)}
                            className="w-full text-left p-2 hover:bg-gray-100 flex items-center gap-2"
                          >
                            {swapCandidate.imageUrl ? (
                              <img
                                src={swapCandidate.imageUrl}
